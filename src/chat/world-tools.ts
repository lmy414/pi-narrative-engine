...
const storyTime = Type.Optional(Type.String());
...
export function createWorldTools(provider: WorldToolsProvider): ToolDefinition[] {
  ...
  return [
    ...
    tool({ name: "world_entity_get", label: "World Entity Get", description: "获取实体快照（bi-temporal：storyTime=故事时间轴，recordedAsOf=事务时间轴）", parameters: Type.Object({ entityId: Type.String(), storyTime, recordedAsOf }), promptSnippet: "获取实体世界图快照", execute: async (p, x: any) => { const st = resolveStoryTime(p, x.storyTime); const snapshot = await p.wg.getEntityAt(x.entityId, st, { recordedAsOf: x.recordedAsOf }); return { content: [{ type: "text", text: snapshot ? JSON.stringify(snapshot) : `未找到实体 ${x.entityId} @ ${st}` }], details: { entityId: x.entityId, storyTime: st, snapshot, error: snapshot ? null : "Entity not found at given storyTime" } }; }}),
    tool({ name: "world_entity_update_summary", label: "World Entity Update Summary", description: "更新实体摘要（作者可见的元信息，纯展示字段，不参与时态/检索/可见性）", parameters: Type.Object({ entityId: Type.String(), summary: Type.String() }), promptSnippet: "更新实体摘要", execute: async (p, x: any) => { await p.wg.updateEntitySummary(x.entityId, x.summary); return { content: [{ type: "text", text: `实体 ${x.entityId} 摘要已更新` }], details: { ok: true, entityId: x.entityId, summary: x.summary } }; }}),
