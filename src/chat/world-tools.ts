...
export function createWorldTools(provider: WorldToolsProvider): ToolDefinition[] {
  ...
  return [
    ...
    tool({ name: "world_entity_create", label: "World Entity Create", description: "创建实体（诞生）", parameters: Type.Object({ entityId: Type.String(), type: entityType, initialProps: Type.Optional(Type.Record(Type.String(), Type.String(), { description: "初始属性（property 用中文词表：角色=名字/性格/背景/说话风格/目标/能力/外貌/位置/心情/健康/当前行动/职业；地点=名字/描述/类型/天气/时段/氛围；物品=名字/材质/主人/历史/能力/状态/位置/磨损；概念=名字/规则/范围/元素；值必须为字符串）" })), storyTime: Type.String() }), promptSnippet: "创建世界图实体", execute: async (p, x: any) => { await p.wg.birthEntity(x.entityId, x.type, x.initialProps ?? {}, x.storyTime); p.setCurrentStoryTime(x.storyTime); return { content: [{ type: "text", text: `实体 ${x.entityId} 已创建（${x.type}）@ ${x.storyTime}` }], details: { ok: true, entityId: x.entityId } }; }}),
    ...
    tool({ name: "world_relation_add", label: "World Relation Add", description: "创建关系", parameters: Type.Object({ /* 🟡 审计修正：主会话版同 agents 版——ID 格式校验 + label 非空（防 rel--label- 垃圾关系） */ sourceId: Type.String({ pattern: "^[A-Za-z0-9_.:-]+$" }), targetId: Type.String({ pattern: "^[A-Za-z0-9_.:-]+$" }), label: Type.String({ description: "关系标签（角色规则集中文枚举：仇敌/朋友/师徒/结义/恋人/上下级/亲属/同盟/敌对/认识/邻居/同事；located_in 为系统保留词勿翻译）", minLength: 1 }), description: Type.Optional(Type.String({ description: "关系叙事描述（可选长句）" })), storyTime: storyTimeOpt }), promptSnippet: "创建世界图关系", execute: async (p, x: any) => { const st = resolveStoryTime(p, x.storyTime); await p.wg.addRelation(x.sourceId, x.targetId, x.label, st, x.description ? { description: x.description } : undefined); return { content: [{ type: "text", text: `关系 ${x.sourceId} -[${x.label}]-> ${x.targetId} 已创建 @ ${st}` }], details: { ok: true, sourceId: x.sourceId, targetId: x.targetId, label: x.label } }; }}),
