...
import type { CastMember, InteractCommand, InteractHooks } from "@pi/role-pool";
...
export async function plan(
  ...
  // 生成 traceId，贯穿本次 dispatch 的所有调试事件（含 yolo 模式下的 commit）
  const traceId = newTraceId();
  const dispatchSpan = startSpan(
    ctx.debugBus,
    "dispatch",
    traceId,
    {
  ...
  try {
    ...
    const planLlmSpan = startSpan(
      ctx.debugBus,
      "plan.llm",
      traceId,
      {
        systemPromptLen: ctx.plannerRuleSet.length,
        userMessage: event.instruction,
        characterCount: event.characterIds.length,
      },
      dispatchSpan.eventId,
    );
    let retrievalPlan;
    try {
      retrievalPlan = await ctx.plannerLlm(
        buildPlannerSystemPrompt(ctx.plannerRuleSet, event),
        buildPlannerUserMessage(event),
      );
      planLlmSpan.end({ itemCount: retrievalPlan.items.length });
    } catch (err) {
      planLlmSpan.error(err);
      throw err;
    }
    ...
    for (const item of retrievalPlan.items) {
      const retrieveSpan = startSpan(
        ctx.debugBus,
        "retrieve.item",
        traceId,
        { type: item.type, params: item.params, label: item.label, assignTo: item.assignTo },
        dispatchSpan.eventId,
      );
      let result;
      try {
        result = await executeRetrievalItem(ctx, item, event.storyTime);
        retrieveSpan.end({ hitCount: result?.length ?? 0 });
      } catch (err) {
        retrieveSpan.error(err);
        // 检索失败不中断整体流程（与原逻辑保持一致）
        result = null;
      }
    ...
    const roleSpan = startSpan(
      ctx.debugBus,
      "role.interact",
      traceId,
      {
        castCount: cast.length,
        characterIds: cast.map((c) => c.characterId),
      },
      dispatchSpan.eventId,
    );
    let roleResult;
    try {
      // 逐角色调试钩子：每个角色的生成挂为 role.interact 的子节点（stage: role.turn）
      // 输入含角色名/顺序/可见事实数；输出含 action/thought/emotion 全文及各类计数
      const roleHooks: InteractHooks | undefined = ctx.debugBus
        ? {
            onTurnStart: (member, index) =>
              startSpan(
                ...
                traceId,
                {
                  ...
                  name: String(member.staticCard.name ?? member.characterId),
                  dynamicFactCount: member.dynamicFacts.length,
                },
                roleSpan.eventId,
              ),
            onTurnEnd: (token, _member, result) => {
              const span = token as ReturnType<typeof startSpan>;
              if (result.error) {
              ...
              span.end({
                actor: out.actor,
                action: out.action,
                ...(out.target !== undefined ? { target: out.target } : {}),
                ...(out.emotion !== undefined ? { emotion: out.emotion } : {}),
                ...(out.thought !== undefined ? { thought: out.thought } : {}),
                stateChangeCount: out.state_changes?.length ?? 0,
                knowledgeGainedCount: out.knowledge_gained?.length ?? 0,
                relationUpdateCount: out.relation_update?.length ?? 0,
              });
            },
          }
        : undefined;
      roleResult = await interact(
        interactCmd,
        {
          llm: ctx.roleLlm,
      ...
      roleSpan.end({
        ...
        errorCount: roleResult.errors.length,
      });
    } catch (err) {
      roleSpan.error(err);
      throw err;
    ...
    //    commit 挂为 dispatch 的子节点，DAG 排布与执行流一致
    if (event.mode === "yolo") {
      const commitResult = await commit(planId, ctx, traceId, dispatchSpan.eventId);
      const output: DispatchYoloOutput = {
        mode: "yolo",
        planId,
        eventId,
        chapterPath,
      ...
      dispatchSpan.end({ mode: "yolo", planId, commitOk: commitResult.ok });
      return output;
    }

    // 10. plan 模式：返回 DispatchPlanOutput（含 retrievalPlan 便于调试）
    //     等主会话审阅后调 scheduler_commit 提交
    const output: DispatchPlanOutput = {
      mode: "plan",
      planId,
      eventId,
      chapterPath,
    ...
    dispatchSpan.end({ mode: "plan", planId });
    return output;
  } catch (err) {
    dispatchSpan.error(err);
    throw err;
  }
}
