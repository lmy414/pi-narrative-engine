...
## 1. 关系类型
- \`knows\`：认识关系（A 认识 B，双向）
- \`located_in\`：位于关系（A 位于 B，如角色在地点、物品在角色身上）
- \`owns\`：持有关系（A 拥有 B，如角色持有物品）
- \`part_of\`：组成关系（A 是 B 的一部分，如地点是更大地点的子区域）
- \`related_to\`：相关关系（兜底，无法用上述表达时使用）
- 自定义 label：仅在基础枚举无法表达时使用（如 "teacher_of" 表示师生关系）
  ...
  "relations": [
    {
      "source_hint": "源实体规范名",
      "target_hint": "目标实体规范名",
      "label": "knows | located_in | owns | part_of | related_to | 自定义",
