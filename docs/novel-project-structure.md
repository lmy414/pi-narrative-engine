...
| 文件 | 归属 | 消费方 |
|------|------|--------|
| `规则集/文风规则.md` | **外部可编辑**（作品文风 + 作品特定格式/禁止约定） | 渲染器子代理（<available_rules> 渐进披露 + `rules_read` 按需读取）；主会话 render_* 工具（全文注入）；旧版 `规则集.md` 兼容回退 |
| `规则集/检查规则.md` | 外部可编辑（校验规则） | render_check（checker） |
| `规则集/自定义规则.md` | 预留（用户/代理可写） | 渐进披露清单可见，渲染器按需读取 |

**文风规则 = 作品级"怎么写"的一切约定**：叙述口吻、句式、描写原则、对话风格、节奏，
**以及作品特定的格式/禁止约定**（句式禁令、词汇禁忌、对话格式偏好、视角限制等）。
引擎内置的通用格式/禁止/输出契约（段落空行、对话「」、不输出标题与元注释、不补缺失维度、不剧透）
是**基础保障兜底**——文风规则中的规则对其**补充与覆盖**，渲染器读取后严格遵守。
...
固化进 `@pi/role-pool` BUILTIN_ROLE_RULES）；渲染**通用**格式/禁止/输出契约（D9，固化进 `@pi/renderer`
RENDERER_SYSTEM_PROMPT——注意：文风规则.md 仍可定义作品特定的格式/禁止约定并覆盖通用兜底）。
