...
**用户澄清（2026-08-10 实施后）**：格式/禁止/输出契约**亦属文风范畴**——文风规则.md 中可定义
作品特定的格式/禁止约定（句式禁令、词汇禁忌、对话格式偏好等），引擎内置的通用格式/禁止/输出契约
仅为**基础保障兜底**，文风规则中的规则对其**补充与覆盖**（novel/ 实盘的文风规则已含「禁止句式」与
「世界观约束」段，即此形态）。模板头部说明与结构文档已按此更新。
