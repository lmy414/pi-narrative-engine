...
## 附录 C：G2-4 测试轮落地 + 后续决策

> 完成日期：2026-08-05
> 提交：`144fc58`（已合并 master 并推送）

### 测试结果

- 19 项清单：通过 12 / 失败 1（BUG-016）/ 跳过 6
- 跳过原因：streaming 边界 3 项（环境未配置 LLM）+ 响应式 2 项（browser_use 无法精确设置窗口宽度）+ 截图 1 项
- 文档：`docs/audits/frontend-test-runs/2026-08-05-sessions.md`

### BUG-016 决策（2026-08-05 用户确认）

**现象**：点击"新建议程"按钮后列表未新增会话项、聊天区未清空；但后续发送消息无 404，疑似 `createChatSession` 请求成功但前端状态未更新或渲染未触发。

**用户决策**：移除"新建议程"按钮功能，统一到对话框内输入意图即可，不需要单独按钮派发。

**处理**：
- BUG-016 标记 `wontfix`，随按钮移除一并解决
- 后端 `POST /api/chat/sessions` 端点保留（对话框意图识别可能复用）
- 前端 `stNewSession` / `createChatSession` api-client 方法保留，按钮 UI 移除
- 移除工作未执行，待后续批次处理（与"重新约定小说项目结构"一起做或独立批次）

### 坑点修复最终状态

| 坑点 | 修复状态 | 说明 |
|---|---|---|
| 坑 1（刷新后历史不可见） | ✅ 已修复 | continueRecent 启动恢复，TC-013 通过 |
| 坑 2（工具调用消失） | ✅ 已修复 | live 指向正确会话，TC-009 通过 |
| 坑 3（新建后旧消息残留） | ⏳ wontfix | BUG-016，随"新建议程"按钮移除一并解决 |
| 坑 4（删除后历史残留） | ⏳ 推后续批次 | G2 不做删除会话 |
