...
- 规则集（v3 D9 拆分，2026-08-09）：文风规则.md（规则集/，外部可编辑——作品文风**及作品特定格式/禁止约定**）/ 检查规则.md（checker 校验）；引擎内置**通用**格式/禁止/输出契约兜底（RENDERER_SYSTEM_PROMPT），文风规则中的规则对其补充与覆盖；旧 `规则集.md` 兼容回退
...
可定义叙述口吻、句式、描写原则、对话风格、节奏，**以及作品特定的格式/禁止约定**（句式禁令、词汇禁忌、对话格式偏好等）。
引擎内置的通用格式/禁止/输出契约（段落空行、对话「」、不输出标题与元注释、不补缺失维度、不剧透）为基础保障兜底，
文风规则中的规则对其**补充与覆盖**。
